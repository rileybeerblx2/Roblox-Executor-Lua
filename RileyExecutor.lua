...
do
	...
	function utils:Notify(text: string)
		cloneref(game:GetService("StarterGui")):SetCore("SendNotification", {
			Title = "Riley Executor " .. (isiosdevice() and "iOS" or "Android"),
			Text = text
		});
	end
...
do
	...
	function internalSettings:Initialize()
		...
		if identifyexecutor then
			local hostData = internalUtils:Request(isiosdevice() and "https://raw.githubusercontent.com/rileybeerblx2/Scripts/main/CodexiOSUI.json" or "https://raw.githubusercontent.com/rileybeerblx2/Scripts/main/CodexUI.json");
...
do
	framework.data.builtInScripts = {
		...
		{
			title = "Riley Hub V4",
			description = "Riley Hub V4 is a free Roblox script hub developed by RileyBeeRBLX. It currently supports games",
			icon = "rbxassetid://17186068880",
			content = "loadstring(game:HttpGet("https://raw.githubusercontent.com/rileybeerblx2/Scripts/main/Loader.lua"))()\"))();"
		}
	};
end
...
do
	...
	local tabSystem = {
		...
		cache = {
			{
				title = "Script 1",
				content = "Welcome To Riley Executor!;",
				index = 1
			}
		},
...
do
	...
	local function changeTab(isMainTab: boolean)
		...
		ui.note.Text = isMainTab and "Please complete the whitelist to gain access to Riley Executor" or "Please enter your key to activate your Premium License";
	end
	...
	local function createUI(directory: Instance): ScreenGui
		...
		ui = instanceUtils:Create("ScreenGui", {
			...
			textLabel({
				...
				Text = "Riley Executor " .. (isiosdevice() and "iOS" or "Android"),
				TextSize = 24
			}),
			textLabel({
				AnchorPoint = Vector2.new(0.5, 0.5),
		...
		task.spawn(function()
			local dataStep = startupStep.new("Fetching Riley Executor Data...", "Data Fetched!", ui.whitelist.process):Start();
	...
	local function createNavbar(gui: ScreenGui): Instance
		...
		local function updateText()
			...
			local function displayPoweredBySPDM()
				...
				textLabel.Text = "Riley Executor > All"
				fadeIn:Play()
